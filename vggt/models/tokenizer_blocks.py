...
class TiTokEncoder(nn.Module):
    def __init__(self, 
        ...
        assert num_register_tokens >= 0, f"num_register_tokens should be non-negative, but got {num_register_tokens}"
    ...
    def forward(self, pixel_values, latent_tokens):
        ...
        if self.register_tokens is not None:
            x = torch.cat([x[:, :1], _expand_token(self.register_tokens, x.shape[0]).to(x.dtype), x[:, 1:]], dim=1)
        ...
        return latent_tokens.squeeze(-1).permute(0, 2, 1).contiguous()  # BLD
