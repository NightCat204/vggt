...
class Trainer:
        ...
        **kwargs,
        ...
        if self.rank == 0:
            ...
            if self._wandb_resume_info:
                ...
                if self._wandb_resume_info.get("project"):
                    wandb_init_kwargs["project"] = self._wandb_resume_info["project"]
                if self._wandb_resume_info.get("entity"):
                    wandb_init_kwargs["entity"] = self._wandb_resume_info["entity"]
                if self._wandb_resume_info.get("id"):
                    wandb_init_kwargs["id"] = self._wandb_resume_info["id"]
    ...
    def train_epoch(self, train_loader, start_iter: int = 0):        
        batch_time = AverageMeter("Batch Time", self.device, ":.4f")
        data_time = AverageMeter("Data Time", self.device, ":.4f")
        mem = AverageMeter("Mem (GB)", self.device, ":.4f")
        data_times = []
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            if self.rank == 0 and wandb.run is not None:
                try:
                    ...
                    wandb.log({"epoch": self.epoch, "train/mean_loss": mean_loss}, step=self.steps[phase])
                except Exception:
                    pass
