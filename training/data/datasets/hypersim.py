...
class HyperSimDataset(BaseDataset):
    ...
    Sequence Structure:
        camera_pose/hypersim/<scene>/*.npz
        rgb/hypersim/<scene>/*.png
        depth_zbuffer/hypersim/<scene>/*.png
        similarity/hypersim/<scene>/dists.npz
    """
    def __init__(
        self,
        common_conf,
        split: str = "train",
        CAMERA_POSE_ROOT: str = "/lustre/fsw/portfolios/nvr/projects/nvr_av_verifvalid/users/ymingli/datasets/Omnidata/omnidata_starter_dataset/camera_pose/hypersim/",
        ...
        logging.info(f"HyperSim CAMERA_POSE_ROOT = {self.CAMERA_POSE_ROOT}")
        ...
        logging.info(f"{status}: HyperSim scene count: {self.sequence_list_len}")
        logging.info(f"{status}: HyperSim dataset length: {len(self)}")
...
if __name__ == "__main__":
    with initialize(version_base=None, config_path="../../config"):
        ...
        dataset = HyperSimDataset(common_conf=cfg.data.train.common_config, split="train", CAMERA_POSE_ROOT="/lustre/fsw/portfolios/nvr/projects/nvr_av_verifvalid/users/ymingli/datasets/Omnidata/omnidata_starter_dataset/camera_pose/hypersim/")
    dataset[(0, 128, 1.0)]